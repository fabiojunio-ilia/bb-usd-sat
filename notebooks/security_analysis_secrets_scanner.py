...
def generate_shared_run_id():
    """
    Generate a shared run_id for this scanner session.
    ...
    """
    try:
        # insertNewBatchRun devolve o id pelo check_time exato do INSERT, nao
        # por max(runID): um driver rodando em paralelo insere na mesma tabela
        # e o max poderia ser o id da sessao dele.
...
def runTruffleHogScanForAllWorkspaces():
    ...
    # Um run_id por SESSAO do scanner, compartilhado por todos os workspaces.
    # A pre-alocacao de um id por workspace criava N linhas na run_number_table
    # por execucao: qualquer leitor de max(runID) (checks do driver,
    # workspace_run_complete, dashboards) podia cair num id que so existia para
    # um workspace e enxergar dados a menos. Notebook e cluster scan continuam
    # correlacionados por (workspace_id, run_id).
    session_run_id = generate_shared_run_id()
    ws_run_ids = [(ws, session_run_id) for ws in scan_workspaces]
